...
//   //         const voiceId = assistantName.toLowerCase() // "slah" or "amira"
...
const speakText = async (text: string) => {
  ...
  // Handle avatar click - toggle recording or stop audio
  const handleAvatarToggleListening = useCallback(() => {
    if (isRecording) {
      stopRecording()
    } else if (!isProcessing && !isPlaying) {
      startRecording()
    }
  }, [isRecording, isProcessing, isPlaying, startRecording, stopRecording])

  // Handle interrupt (stop audio when speaking)
  const handleAvatarInterrupt = useCallback(() => {
    if (isPlaying) {
      stopAudio()
    }
  }, [isPlaying, stopAudio])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 pt-20">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {/* Avatar - Now the main interaction point */}
        <div className="flex justify-center mb-8">
          <Avatar
            assistantId={assistantId}
            isListening={isRecording}
            ...
            hasConversationStarted={conversationHistory.length > 0}
            status={getStatus(isRecording, isProcessing, isPlaying, conversationHistory.length)}
            onToggleListening={handleAvatarToggleListening}
            onInterrupt={handleAvatarInterrupt}
          />
        </div>
