...
export function Avatar({
  ...
  // Handle avatar click based on current state
  ...
  const getCursorStyle = () => {
    const state = getAvatarState()
    if (state === "processing") return "cursor-wait"
    return "cursor-pointer"
  }

  // Get hint text for the current state
  const getHintText = () => {
    const state = getAvatarState()
    switch (state) {
      case "idle":
        return t("status.clickToStart") || "Click to start talking"
      case "listening":
        return t("status.clickToStop") || "Click to stop"
      case "speaking":
        return t("status.clickToInterrupt") || "Click to stop"
      case "processing":
        return t("voice.thinking") || "Processing..."
      default:
        return ""
    }
  }
      ...
      <div className="relative">
        ...
        <button
          ...
          className={`
            ...
            ${isSpeaking ? "ring-4 ring-red-300 ring-opacity-50 animate-pulse" : ""}
            ${isProcessing ? "opacity-80" : ""}
            ${!isProcessing && !isListening && !isSpeaking ? "hover:ring-4 hover:ring-blue-300 hover:ring-opacity-50" : ""}
          `}
          style={{
            transform: isListening || isSpeaking ? `scale(${1 + (pulseIntensity / 100) * 0.1})` : "scale(1)",
          }}
